In this third practical session, we will learn how to create synthetic graphical representations using {{< fa brands r-project >}}, which is very well equipped in this field thanks to the `ggplot2` library. The latter implements a [grammar of graphics](https://fr.wikibooks.org/wiki/Programmer_en_R/Comprendre_la_grammaire_des_graphiques) that is flexible, coherent, and easy to use.

If you're interested in `Python` {{< fa brands python >}}, a very similar version of this session is available in [my ENSAE course](https://pythonds.linogaliana.fr/content/visualisation/matplotlib.html).

::: {.callout-note}

Some code examples include annotations on the side. Hover your mouse over them to display, like below:
...
* Primarily the [`ggplot2`](https://ggplot2.tidyverse.org/) package for building static graphs;
* The [`plotly`](https://plotly.com/r/) package for dynamic, HTML-format graphs. We will deepen our learning of HTML graphs in a future chapter by exploring [`Observable`](https://observablehq.com/).
...
We will later see how to easily create maps with equivalent formats.
...
We propose to download this data and save it in a file on the local hard drive before importing it[^1]. However, we will not do this manually but rather use {{< fa brands r-project >}}. Doing such an action manually would be a poor practice from a reproducibility perspective.
